...
function read_credentials_sync(obj = {}) {
  ...
  if (result[android_application_id_key] !== cache_android_app_id || forceBundleUpdate) {
    if (forceBundleUpdate) {
      ...
      console.log(
        `[CREDENTIALS] Android Application ID changed from "${cache_android_app_id}" to "${result[android_application_id_key]}", updating build.gradle...`,
      );
    }
    write_android_application_id(result[android_application_id_key]);
  } else {
    console.log(
      `[CREDENTIALS] Android Application ID unchanged (${result[android_application_id_key]}), because no update needed`,
    );
  }
...
function write_android_application_id(new_app_id) {
  ...
  try {
    ...
    var buildGradleContentNew = buildGradleContent.replace(/applicationId\s+"[^"]+"/g, `applicationId "${new_app_id}"`);
