...
import { OnboardingEventId, type OnboardingEventView, type ParsedOnboardingEvent } from '../types/onboarding-events';

// Parser
export function parseOnboardingEvent(input: string, ctx?: LogContext): ParsedOnboardingEvent | null {
  let obj: Record<string, unknown>;
  try {
    obj = JSON.parse(input);
  } catch (error) {
    throw AdaptyError.failedToDecode(`Failed to decode event: ${(error as Error)?.message}`);
  }
  ...
  const decodeMeta = () => getOnboardingCoder('meta', ctx)!.decode(obj['meta']) as AdaptyUiOnboardingMeta;
  ...
  switch (eventId) {
    ...
    case OnboardingEventId.StateUpdated:
      return {
        id: eventId,
        view,
        action: getOnboardingCoder('action', ctx)!.decode(obj['action']) as OnboardingStateUpdatedAction,
        meta: decodeMeta(),
      };
...
function getOnboardingCoder(type: OnboardingCoderType, _ctx?: LogContext): Converter<any, any> | ErrorConverter<any> {
  switch (type) {
    case 'meta':
