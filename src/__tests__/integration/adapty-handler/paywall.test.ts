...
describe('Adapty - Paywall (Bridge Integration)', () => {
  ...
  describe('Basic getPaywall', () => {
    ...
    it('should return different paywalls for different placementId', async () => {
      ...
      nativeMock.handleMethodCall.mockImplementationOnce((_methodArgs: { methodName: string; args: string }) => {
        return Promise.resolve({
          crossPlatformJson: JSON.stringify(GET_PAYWALL_RESPONSE),
        });
      });
      ...
      const paywall1 = await adapty.getPaywall({ placementId: 'placement_one' });
      const paywall2 = await adapty.getPaywall({ placementId: 'placement_two' });
    ...
    it('should return paywall with correct structure', async () => {
      const paywall = await adapty.getPaywall({ placementId: 'structure_test' });

      // Verify: request sent with correct placement_id
      const request = extractNativeRequest<components['requests']['GetPaywall.Request']>({
        nativeModule: nativeMock,
      });
      ...
      expect(paywall.paywallBuilder !== undefined || paywall.remoteConfig !== undefined).toBe(true);
  ...
  describe('getPaywallForDefaultAudience', () => {
    ...
    it('should include locale when provided', async () => {
      await adapty.getPaywallForDefaultAudience({
        placementId: 'test_placement_default',
        locale: 'ru',
      });

      const request = extractNativeRequest<components['requests']['GetPaywallForDefaultAudience.Request']>({
        nativeModule: nativeMock,
      });
  ...
  describe('logShowPaywall', () => {
    it('should log paywall show event after getPaywall', async () => {
      ...
      const request = extractNativeRequest<components['requests']['LogShowPaywall.Request']>({
        nativeModule: nativeMock,
      });
  ...
  describe('Error handling', () => {
    it('should throw on GetPaywall.Response error', async () => {
      // Reset and create mock with error response for get_paywall
      resetNativeModuleMock(nativeMock);
      nativeMock = createNativeModuleMock({
        activate: ACTIVATE_RESPONSE_SUCCESS,
      ...
      // Execute: get paywall should throw with error message from response
      await expect(adapty.getPaywall({ placementId: 'nonexistent_placement' })).rejects.toThrow('Paywall not found');
    });
  });
});
