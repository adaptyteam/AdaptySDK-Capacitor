...
describe('Adapty - Profile', () => {
  ...
  describe('Profile update and retrieval', () => {
    it('should send UpdateProfile in snake_case and return only customAttributes', async () => {
      ...
      const updateRequest = extractNativeRequest<components['requests']['UpdateProfile.Request']>({
  ...
  describe('Error handling', () => {
    it('should parse AdaptyError from UpdateProfile.Response', async () => {
      // Reset and create new mock with error response for update_profile
      resetNativeModuleMock(nativeMock);
      nativeMock = createNativeModuleMock({
        activate: ACTIVATE_RESPONSE_SUCCESS,
      ...
      await expect(adapty.updateProfile({ email: 'test@example.com' })).rejects.toThrow('Profile update failed');
    });
  });
});
