...
**CI verification:** `scripts/verify-kids-mode-ios.sh <on|off|both>` builds `examples/adapty-devtools` with the trait toggled and inspects every Mach-O in `App.app` (`otool`/`nm`) to prove AdSupport/IDFA is compiled out. The workflow `.github/workflows/kids-mode-ios.yml` runs it with `on`; run `both` locally for the positive + negative control. ATT is currently excluded — AdaptySDK-iOS 4.0.0 imports `AppTrackingTransparency` unconditionally in `AdaptyProfileParameters.Builder` (not gated by `!KidsMode`); re-enable in the two patterns when the SDK ships the fix.
